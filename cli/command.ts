...
programCommand('update_fee')
  ...
  .option(
    '-t, --token_fee <number>',
    'marketplace trading by token fee as permyraid'
  )
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  .action(async (directory, cmd) => {
    const { env, sol_fee, token_fee } = cmd.opts();
...
programCommand('deposit')
  ...
  .action(async (directory, cmd) => {
    ...
    await depositEscrow(
      parseFloat(sol) * LAMPORTS_PER_SOL,
      parseFloat(token) * ABB_TOKEN_DECIMAL
    );
  });
...
programCommand('list')
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  .action(async (directory, cmd) => {
    const { env, address, price_sol, price_token } = cmd.opts();
    ...
    await listNftForSale(
      ...
      parseFloat(price_token) * ABB_TOKEN_DECIMAL
    );
  });
...
programCommand('purchase')
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  .action(async (directory, cmd) => {
    const { env, address, by_token } = cmd.opts();
...
programCommand('make_offer')
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  .action(async (directory, cmd) => {
    const { env, address, price, by_token } = cmd.opts();
    ...
    await makeOffer(
      new PublicKey(address),
      parseFloat(price) * (byToken ? ABB_TOKEN_DECIMAL : LAMPORTS_PER_SOL),
      byToken
    );
  });
...
programCommand('create_auction')
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  .action(async (directory, cmd) => {
    const { env, address, start_price, min_increase, end_date, by_token } =
      cmd.opts();
    ...
    await createAuction(
      new PublicKey(address),
      parseFloat(start_price) *
        (byToken ? ABB_TOKEN_DECIMAL : LAMPORTS_PER_SOL),
      parseFloat(min_increase) *
        (byToken ? ABB_TOKEN_DECIMAL : LAMPORTS_PER_SOL),
      parseInt(end_date),
      byToken
    );
