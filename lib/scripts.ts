...
export const createUpdateFeeTx = async (
  ...
  tx.add(
    program.instruction.updateFee(
      bump,
      new anchor.BN(solFee),
      new anchor.BN(tokenFee),
      {
        accounts: {
          admin: userAddress,
          globalAuthority,
...
export const createAddTreasuryTx = async (
  ...
  console.log(
    'Treasury ABB Account = ',
    ret1.destinationAccounts[0].toBase58()
  );
...
export const createDepositTx = async (
  ...
  if (!(await isExistAccount(userTokenAccount, connection))) {
    try {
      let accountOfABB = await getTokenAccount(
        ABB_TOKEN_MINT,
        userAddress,
        connection
      );
...
export const createPurchaseTx = async (
  ...
  if (byToken) {
    for (let idx in treasuryAccounts) {
      treasuryAccounts[idx] = await getAssociatedTokenAccount(
        treasuryAccounts[idx],
        ABB_TOKEN_MINT
      );
    }
  }
...
export const createAcceptOfferTx = async (
  ...
  let ret1 = await getATokenAccountsNeedCreate(connection, seller, seller, [
    ABB_TOKEN_MINT,
  ]);
...
export const createPlaceBidTx = async (
  ...
  console.log(
    ...
    price,
    'ByToken:',
    auctionInfo.byToken.toNumber(),
    'LastBidder:',
    outBidder.toBase58(),
    'LastBidderATA:',
    outBidderTokenAccount.toBase58()
  );
  tx.add(
